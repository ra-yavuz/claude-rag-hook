"""MCP server (stdio JSON-RPC) for claude-rag-hook.
...
_TOOL_RAG_SEARCH_DESCRIPTION = (
    ...
    "claude-rag-hook. Use this when the user's question would be better "
...
_TOOL_RAG_LIST_DESCRIPTION = (
    ...
    "claude-rag-hook index, with file/chunk counts and any tags. Useful "
...
def _tool_rag_search(args: dict[str, Any]) -> dict[str, Any]:
    ...
    lines = [
        f"Retrieved {len(hits)} chunk(s) from claude-rag-hook local index "
...
def _tool_rag_status(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if idx is None:
        return _text_result(
            f"rag_status: no claude-rag-hook index found in or above {cwd}. "
...
def serve(stdin=None, stdout=None) -> int:
    ...
    print(
        f"claude-rag-mcp v{SERVER_VERSION} ready (stdio).",
    ...
    while True:
        ...
        try:
            ...
            print(f"claude-rag-mcp: invalid JSON: {e}", file=sys.stderr, flush=True)
            continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    if os.environ.get("CLAUDE_RAG_MCP_DISABLED") == "1":
        print("claude-rag-mcp: disabled by user (CLAUDE_RAG_MCP_DISABLED=1).",
              file=sys.stderr, flush=True)
        return 0
    return serve()
