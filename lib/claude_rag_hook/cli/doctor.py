...
def _check_managed_settings_wired() -> dict:
    ...
    if not p.exists():
        return _check(
            ...
            f"Use `claude-rag-hook-admin install` if you installed by hand.",
        )
    try:
    ...
    return _check(
        "hook wired into managed-settings.json",
        _FAIL,
        f"no UserPromptSubmit entry referencing claude-rag-hook in {p}",
    )


def _check_hook_binary() -> dict:
    p = Path("/usr/lib/claude-rag-hook/claude-rag-hook-hook")
    if not p.is_file():
...
def _check_systemctl_user() -> dict:
    ...
    try:
        out = subprocess.run(
            ["systemctl", "--user", "is-active", "claude-rag-hook-refresher.service"],
